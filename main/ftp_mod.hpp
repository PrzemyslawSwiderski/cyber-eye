...
namespace ftp
{
  inline std::unique_ptr<espp::FtpServer> ftp_server;
  inline espp::Logger logger({.tag = "FTP", .level = espp::Logger::Verbosity::DEBUG});

  inline void start_server_task(const std::string &ip_address)
  {
    if (ftp_server)
    {
